...
import com.tom.chef.utils.Utilities.loadBitmapFromView
...
//            mTarget.setVisibility(View.INVISIBLE);
    ...
    private val avatarRevealAnimator:

//        float scaleFactor = Math.max(2f * destY / originY, 2f * destX / originX);
            AnimatorSet
        private get() {
//        final float endRadius = Math.max(destX, destY) / 2;
            val endRadius = Math.max(90, 90).toFloat()
...
//        Bitmap dest = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        val dest = loadBitmapFromView(view!!, view.width, view.height)
        val c = Canvas(dest)
