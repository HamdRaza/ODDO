...
object Validation {
    ...
    fun isAgree(activity: Activity, checkBox: CheckBox): Boolean {
        return if (checkBox.isChecked) {
            ...
            activity.myToast(activity.resources.getString(R.string.pleaseagree))
            false
        }
    }
...
//        if(!checkIsEmpty(editText)) {
        if (Patterns.EMAIL_ADDRESS.matcher(editText.text.toString().trim { it <= ' ' }).matches()) {
            ...
            editText.error = editText.resources.getString(R.string.err_email_valid)
...
//        }
    ...
    fun isAValidPassword(vararg editText: TextView): Boolean {
        for (ed in editText) {
            val text = ed.text.toString()
            return if (text.length < 8 || !JavaUtilities.passwordCharValidation(text)) {
    ...
    fun isAvalidName(editText: View): Boolean {
        val isValid = namespecialChars.matcher(editText.getLocalText()).find()
        if (!isValid) {
             editText.setLocalError(editText.context.getString(R.string.err_name))
             editText.requestFocus()
        } else {
            editText.setLocalError(null)
        }
        return isValid
    }
...
//            editText.setError(editText.getResources().getString(R.string.err_special_characters));
    ...
    fun isPasswordMatch(password: TextView, retype: TextView,isNewPassword:Boolean=false): Boolean {
        var check = true
        ...
        if (password.text.toString() != retype.text.toString()) {
            if (isNewPassword){
                retype.error = retype.resources.getString(R.string.err_pass_match_new)
            }else{
